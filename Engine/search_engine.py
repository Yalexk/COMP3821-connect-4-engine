...
class SearchEngine:
    ...
    def search_root(self, board: Board, depth: int, ctx: SearchContext):
        ...
        for m in moves:
            board.play(m, 1)
            score, searched = self.search(board, depth - 1, -10**9, 10**9, False, ctx)
            board.undo(m)
        ...
        return best_move, best_score, n_searched
    ...
    def search(self, board: Board, depth, alpha, beta, maximizing, ctx: SearchContext):
        ...
        if ctx.use_tt:
            ...
            if score is not None:
                return score, 1  # exact score usable
        ...
        if maximizing:
            ...
            for m in moves:
                board.play(m, 1)
                score, searched = self.search(board, depth - 1, alpha, beta, False, ctx)
                board.undo(m)
            ...
            for m in moves:
                board.play(m, -1)
                score, searched = self.search(board, depth - 1, alpha, beta, True, ctx)
                board.undo(m)
    ...
    def iterative_deepening(self, board: Board, ctx: SearchContext):
        ...
        for d in range(1, ctx.max_depth + 1):
            move, score, searched = self.search_root(board, d, ctx)
            if ctx.time_exceeded():
                break
            if move is not None:
